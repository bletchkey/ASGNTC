...
def retrieve_log_data(log_path: str):
    ...
    train_pred_scores_pattern = re.compile(r"Prediction scores P \[train: ([0-9.]+)%, val: ([0-9.]+)%\]")
    ...
    for line in log.split('\n'):
        ...
        train_pred_scores_match = train_pred_scores_pattern.search(line)

        if epoch_match and train_loss_match and train_pred_scores_match:
            epochs.append(int(epoch_match.group(1)))
            train_losses.append(float(train_loss_match.group(1)))
            val_losses.append(float(train_loss_match.group(2)))
            train_prediction_scores.append(float(train_pred_scores_match.group(1)))
            val_prediction_scores.append(float(train_pred_scores_match.group(2)))
    ...
    training_data = {
        ...
        "train_prediction_scores" : train_prediction_scores,
        "val_prediction_scores"   : val_prediction_scores,
