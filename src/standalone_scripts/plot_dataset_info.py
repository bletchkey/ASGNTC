...
def check_dataset_configs(saving_path, dataset, metadata, total_indices=200, n_per_png=20):
    ...
    for png_idx in range(total_indices // n_per_png):
        ...
        gs = GridSpec(n_per_png, 6, figure=fig, height_ratios=[6, 1], hspace=0.1, wspace=0.1)
        ...
        for config_idx in range(n_per_png):
            ...
            for img_idx in range(6):
                ...
                if img_idx == 0:
                    ax.text(0.1, 0, metadata_texts[img_idx], ha='center', va='center', fontsize=14, color='black', wrap=True)
                    ...
                    ax.set_title(titles[img_idx], loc='center', fontsize=20, pad=20)
                    ax.axis('off')
                    ax.text(0.1, 0, metadata_texts[img_idx], ha='left', va='top', fontsize=14, color='black', transform=ax.transAxes, wrap=True)

        # Adjust layout for padding and spacing
        plt.subplots_adjust(left=0.05, right=0.95, top=0.4, bottom=0.1, wspace=0.1, hspace=0.4)
