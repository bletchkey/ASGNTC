...
def check_dataset_distribution(device, saving_path):
    ...
    ds_path = {
        "train": os.path.join(constants.dataset_path, str(constants.dataset_name+"_train.pt")),
        "val": os.path.join(constants.dataset_path, str(constants.dataset_name+"_val.pt")),
        "test": os.path.join(constants.dataset_path, str(constants.dataset_name+"_test.pt"))
    }

    plot_train = os.path.join(saving_path, "train_dist.png")
    plot_val   = os.path.join(saving_path, "val_dist.png")
    plot_test  = os.path.join(saving_path, "test_dist.png")
...
def check_dataset_configs(saving_path, dataset, metadata, total_indices=200, n_per_png=20):
    ...
    for png_idx in range(total_indices // n_per_png):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(saving_path, f"configs_{png_idx+1}.png"), bbox_inches='tight')
        plt.close(fig)
...
def main():
    ...
    base_saving_path = os.path.join(constants.dataset_path, "plots")
    ...
    for name in names:
        ...
        saving_path = os.path.join(base_saving_path, name)
        os.makedirs(saving_path, exist_ok=True)

        ds_path = os.path.join(constants.dataset_path, f"{constants.dataset_name}_{name}.pt")
        metadata_path = os.path.join(constants.dataset_path, f"{constants.dataset_name}_metadata_{name}.pt")
